...
def run():
    ...
    #new_dict = dict()
    starttime = time.time()
    #process_logread = subprocess.Popen(["logread.sh"],shell=True,preexec_fn=os.setsid)
    #process_idleloop = subprocess.Popen(["idleloop.sh"],shell=True,preexec_fn=os.setsid)
    ...
    #os.killpg(os.getpgid(process_idleloop.pid), signal.SIGKILL)
    #os.killpg(os.getpgid(process_logread.pid), signal.SIGKILL)
...
if __name__ == "__main__":
    ...
    #proc = subprocess.Popen(['./idleloop.sh']) 
    run()
    #proc.kill()
